class HooAddonsUtil {
    constructor() {
        if (HooAddonsUtil.instance) return HooAddonsUtil.instance;
    ...
    getInstance() {
        ...
        HooAddonsUtil.instance = instance;
        return instance;
    }
}
//创建一个音乐播放器的类 单例模式

class HooAddonsPlayer {
    constructor() { //类的构造函数
        //如果类已经有实例了，就返回这个实例
        if (HooAddonsPlayer.instance) return HooAddonsPlayer.instance;
        //如果没有实例化，就去构造一个实例
        return this.getInstance(...arguments);
    }
    ...
    //构建实例
    getInstance() {
        let instance = new HooAddonsPlayerCreator(...arguments);
        //让实例可以使用到HooAddonsPlayer的原型的属性方法
        // instance.__proto__=HooAddonsPlayer.prototype;
        // instance.constructor=HooAddonsPlayer;
        //把构建好的实例挂在HooAddonsPlayer类上
        HooAddonsPlayer.instance = instance;
        return instance;
    }
}

//歌曲信息
class HooAddonsMusics {
    //歌曲
    constructor() {
        this.songs = hoo_addons_music
    }
    //根据索引获取歌曲的方法
    getSongByNum(index) {
...
class HooAddonsPlayerCreator {
    constructor() {
        ...
        //工具
        this.util = new HooAddonsUtil();
        this.musics = new HooAddonsMusics(); //歌曲信息
        this.song_index = 0; // 当前播放的歌曲索引
        this.loop_mode = 0; // 1 2
        // 下方歌曲列表容器
        this.song_list = jQuery('.music__list_content');

        this.render_doms = { //切换歌曲时需要渲染的dom组
            title: jQuery('.music__info--title'),
            singer: jQuery('.music__info--singer'),
            image: jQuery('.music-player__image img'),
            blur: jQuery('.music-player__blur')
        }
        this.ban_dom = { //禁音时需要渲染的dom组
            control__btn: jQuery('.control__volume--icon')
        }

        // 时间显示容器
        this.render_time = {
            now: jQuery('.nowTime'),
            total: jQuery('.totalTime')
        }

        // 唱片
        this.disc = {
            image: jQuery('.music-player__image'),
            pointer: jQuery('.music-player__pointer')
        };
        //播放器初始化
        this.init();
    }
    ...
    bindEventListener() {
        //播放按钮
        this.$play = new HooAddonsBtns('.player-control__btn--play', {
            click: this.handlePlayAndPause.bind(this)
        });
        //上一首
        this.$prev = new HooAddonsBtns('.player-control__btn--prev', {
            click: this.changeSong.bind(this, 'prev')
        });
        //下一首
        this.$next = new HooAddonsBtns('.player-control__btn--next', {
            click: this.changeSong.bind(this, 'next')
        });
        //循环模式
        this.$mode = new HooAddonsBtns('.player-control__btn--mode', {
            click: this.changePlayMode.bind(this)
        });
        //禁音
        this.$ban = new HooAddonsBtns('.control__volume--icon', {
            click: this.banNotes.bind(this)
        })
        //列表点击
        this.song_list.on('click', 'li', (e) => {
            let index = jQuery(e.target).index();
            this.changeSong(index);
        })

        //音量控制 audio标签音量 vlouem 属性控制0-1

        new HooAddonsProgress('.control__volume--progress', {
            min: 0,
            max: 1,
        ...
        this.audio.oncanplay = () => {
            ...
            this.progress = new HooAddonsProgress('.player__song--progress', {
                min: 0,
    ...
    songTime() {
        ...
        jQuery('.totalTime').text(totalMinute + ':' + totalSecond);
    }
    //切换歌曲
    changeSong(type) {
        //更改索引
...
class HooAddonsProgress {
    ...
    renderBackAndPointer() {
        this.$back = jQuery('<div class="back">');
        this.$pointer = jQuery('<div class="pointer">');
...
if( typeof hoo_addons_music != 'undefined' ) new HooAddonsPlayer({'audio':hoo_addons_music});